...
- 새 테이블을 만드는 마이그레이션은 **같은 파일에서** `ENABLE ROW LEVEL SECURITY` 한다. 정책은 만들지 않는다 (030 이후 anon/authenticated 차단 유지).
- 마이그레이션 작성 후 `docs/db/sql/check/rls_status.sql`로 RLS·권한을 확인한다.
...
- 청구 배치 자동 실행(cron) 연결
- 토스페이먼츠 실결제 연동
- 보안 ⑤ RLS: 030 실행·E2E S17 통과 (2026-09-27_0139)
