...
export default function WritePostPage({ params }) {
  ...
  async function handleSubmit(e) {
    ...
    router.push(sitePublicPath(siteCode, '/board'))
    router.refresh()
  }
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#fafaf9', fontFamily: "'Georgia', serif" }}>
        <header style={{
          ...
          <Link href={sitePublicPath(siteCode)} style={{ color: 'white', textDecoration: 'none', fontSize: 20, fontWeight: 600 }}>
            {site?.name || siteCode}
          </Link>
          <nav style={{ display: 'flex', gap: 28 }}>
            <Link href={sitePublicPath(siteCode)} style={{ color: '#d6d3d1', textDecoration: 'none', fontSize: 14 }}>홈</Link>
            <Link href={sitePublicPath(siteCode, '/board')} style={{ color: '#d6d3d1', textDecoration: 'none', fontSize: 14 }}>게시판</Link>
          </nav>
        </header>
        <div style={{ maxWidth: 480, margin: '0 auto', padding: '80px 20px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            ...
            <Link href={`/login?next=${encodeURIComponent(sitePublicPath(siteCode, '/board/write'))}`} style={{
    ...
    <div style={{ minHeight: '100vh', background: '#fafaf9', fontFamily: "'Georgia', serif" }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '48px 20px' }}>
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          {[
            { key: 'author', label: '작성자', ph: '이름 (비워두면 운영자)', required: false },
