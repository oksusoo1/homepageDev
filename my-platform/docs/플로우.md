...
flowchart LR
  ...
  Choose -->|self| A1[building] --> A3[pay_method] --> A4[trial] --> A5[subscribed]
...
**카드 결제 시점:** 체험 전·중 등록은 결제하지 않는다(첫 결제는 체험 종료일 청구 배치).  
해지 후 재구독(`cancelled_at` 또는 `sites.status=suspended`)일 때만 등록 즉시 1개월분.

해지 예정일 도래 쓰기는 청구 배치에서만 (`lib/billing-batch.js`). 방문자 조회는 해지일 기준 hidden 판단만.
탈퇴 예약일(`customers.withdraw_at`) 도래도 청구 배치에서만 — 사이트 `suspended` · 고객 `withdrawn`. `/my`는 상태 표시만.
...
| 쓰기(③) | Server Action + `lib/supabase/admin.js`. 사장님 사이트 수정은 `content`·이름·표시용 텍스트만 (`lib/site-edit.js`). 주소 규칙 `lib/subdomain-rules.js` |
| 방문자 읽기(④) | `lib/public/*` + 서버 게이트. 비공개·비밀글·문의 연락처는 응답에 넣지 않음. `dynamic=force-dynamic` |
| 남은 조회·잠금(⑤) | `/my`·admin·에디터 조회는 Server Action (`lib/server/reads.js`). 브라우저 supabase는 auth만. RLS 잠금은 `030_rls_lockdown.sql` (사용자 실행) |
