...
export async function loadPublicPost(board, postId, viewer, ownerCustomerId) {
  const db = createAdminClient()
  const postRes = await onlyActive(
    db.from('user_posts')
  ...
  const canRead = canReadSecretPost(post, viewer, ownerCustomerId)
  const commentsQuery = canRead
    ? onlyActive(
      db.from('user_comments').select(COMMENT_COLS).eq('post_id', postId).order('created_at')
    )
    : onlyActive(
      db.from('user_comments').select('author_type, use_flag').eq('post_id', postId)
    )
  const { data, error } = await commentsQuery
  if (error) throw error
  return presentPostDetail(post, data || [], viewer, ownerCustomerId)
}
